...
const sortedCategories = useMemo(() => {
  ...
  const rest = categories.filter((c) => c.name !== "Inne").sort((a, b) => a.name.localeCompare(b.name, "pl"));
...
export function useTransactionMutations() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateTransactionCommand) => createTransaction(data),
...
```typescript
export function useTransactionForm(mode: "create" | "edit", transaction?: TransactionDto, defaultDate?: string) {
...
export const transactionFormSchema = z.object({
  ...
  categoryId: z.string({ required_error: "Kategoria jest wymagana" }).uuid("Nieprawidłowy identyfikator kategorii"),
